...
import { Attachable, AttachableProvider } from './attachable.js'
...
export function SwitchCase({
  ...
  const sequential = useContext(SwitchContext)
  if (sequential == null) {
  ...
  const attachable = useMemo<Attachable>(
    () => ({
      attach: (timeline) =>
        sequential.attach(index, (...params) => conditionRef.current?.(...params) ?? true, timeline),
