...
function Scene() {
  ...
  useTimeline(async function* () {
    while (true) {
      yield* parallel('all', moveTo(cubeA, new Vector3(-3.5, 0.5, 0)), moveTo(bluePill, new Vector3(1.5, -0.5, 0)))
      // Then nudge both back to origin, racing the first finisher
      yield* parallel('all', moveTo(cubeB, new Vector3(0, 0, 0)), moveTo(bluePill, new Vector3(0, 0, 0)))
    }
  }, [])
...
function Scene() {
  ...
  useTimeline(async function* () {
    while (true) {
      yield* parallel('all', moveTo(redPill, new Vector3(-3.5, 0.5, 0)), moveTo(bluePill, new Vector3(1.5, -0.5, 0)))
      // Then nudge both back to origin, racing the first finisher
      yield* parallel('all', moveTo(redPill, new Vector3(0, 0, 0)), moveTo(bluePill, new Vector3(0, 0, 0)))
    }
  }, [])
