---
title: Overview over All Functions.
description: Overview over all types of functions (action update, action until, action update ease)
nav: 3
---
...
#### Parameters
- \`config\` ({ daming: number; mass: number; maxVelocity?: number; stiffness: number }): allows to configure the physical properties of the spring (available presents are `springPresets.gente`, `springPresets.wobbly`, `springPresets.stiff`) - default is `springPresets.gente`
#### Returns
